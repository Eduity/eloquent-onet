...
class InstallOnetDatabase extends Migration
{
    ...
    public function up()
    {
    	$this->downloadLatestArchive();
    	$this->importLatestArchive();
    	$this->renameTables();
    }

    public function down()
    {
    	$this->deleteOnetTables();
    }

    public function downloadLatestArchive()
    {
		$this->console->writeln("<info>Downloading " . config('eloquent-onet.download_url') . "...</info>");
    	$filename = basename(config('eloquent-onet.download_url'));
    	$filepath = 'onet/' . $filename;

    	if(Storage::exists($filepath)) {
    		// Already downloaded.
			$this->console->writeln("<info>" . config('eloquent-onet.download_url') . " already available.</info>");
    		return;
    	}
    	...
    	if(Storage::has($filepath)) {
			$this->console->writeln("<info>Saved " . storage_path($filepath) . "...</info>");
    	}
    }
	...
	public function importDataFromStorage($directory)
	{
		...
		if(count($files)) {
			...
			// DB::unprepared('SET GLOBAL net_buffer_length=100000000;'); 
			// DB::unprepared('SET GLOBAL max_allowed_packet=100000000000;');
			// DB::unprepared('SET GLOBAL wait_timeout=10;');
